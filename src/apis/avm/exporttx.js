...
var ExportTx = /** @class */ (function (_super) {
    ...
    function ExportTx(networkID, blockchainID, outs, ins, memo, destinationChain, exportOuts) {
        ...
        _this._typeID = _this._codecID === 0 ? constants_1.AVMConstants.EXPORTTX : constants_1.AVMConstants.EXPORTTX_CODECONE;
        _this.destinationChain = undefined;
        _this.numOuts = buffer_1.Buffer.alloc(4);
        _this.exportOuts = [];
        _this.destinationChain = destinationChain; // no correction, if they don"t pass a chainid here, it will BOMB on toBuffer
        if (typeof exportOuts !== "undefined" && Array.isArray(exportOuts)) {
            for (var i = 0; i < exportOuts.length; i++) {
                if (!(exportOuts["".concat(i)] instanceof outputs_1.TransferableOutput)) {
                    throw new errors_1.TransferableOutputError("Error - ExportTx.constructor: invalid TransferableOutput in array parameter ".concat(exportOuts));
                }
            }
     ...
     */
    ExportTx.prototype.setCodecID = function (codecID) {
